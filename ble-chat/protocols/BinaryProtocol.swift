...
let logger = Logger(subsystem: "com.bitcli.ble", category: "main")
...
struct BinaryProtocol {
    ...
    static func encode(_ packet: BlePacket, padding: Bool = true) -> Data? {
      ...
      let senderBytes = packet.senderID.data(using: .utf8) ?? Data()
      var senderPadded = Data(count: senderIDSize)
      let senderCopyLength = min(senderBytes.count, senderIDSize)
      senderPadded.replaceSubrange(0..<senderCopyLength, with: senderBytes.prefix(senderCopyLength))
      data.append(senderPadded)

      // Recipient ID (8 bytes, if present)
      if let recipientId = packet.recipientID, !recipientId.isEmpty {
          let recipientBytes = recipientId.data(using: .utf8) ?? Data()
          var recipientPadded = Data(count: recipientIDSize)
          let recipientCopyLength = min(recipientBytes.count, recipientIDSize)
          recipientPadded.replaceSubrange(0..<recipientCopyLength, with: recipientBytes.prefix(recipientCopyLength))
          data.append(recipientPadded)
      }
    ...
    static func decode(_ data: Data) -> BlePacket? {
    ...
    private static func decodeCore(_ raw: Data) -> BlePacket? {
        guard raw.count >= v1HeaderSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> BlePacket? in
            ...
            guard let senderIdBytes = readData(senderIDSize) else { return nil }

          // Convert sender ID bytes to string, removing null padding
          let senderIdEnd = senderIdBytes.firstIndex(of: 0) ?? senderIdBytes.count
          let senderId = String(data: senderIdBytes.prefix(senderIdEnd), encoding: .utf8) ?? ""

          // Read recipient ID (if present)
          var recipientId: String? = nil
          if hasRecipient {
              guard let recipientIdBytes = readData(recipientIDSize) else { return nil }
              
              // Convert recipient ID bytes to string, removing null padding
              let recipientIdEnd = recipientIdBytes.firstIndex(of: 0) ?? recipientIdBytes.count
              recipientId = String(data: recipientIdBytes.prefix(recipientIdEnd), encoding: .utf8)
          }
            ...
            if isCompressed {
                ...
                guard compressionRatio <= 50_000.0 else {
                    logger.warning("🚫 Suspicious compression ratio: \(String(format: "%.0f", compressionRatio)):1")
                    return nil
                }

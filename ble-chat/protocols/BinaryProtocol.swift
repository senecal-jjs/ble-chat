  ...
  static func decode(_ data: Data) -> Packet? {
    ...
    if isCompressed {
      ...
      let originalSize = Int(
        originalSizeData.reduce(0) { result, byte in
          (result << 8) | UInt16(byte)
        })
      offset += 2
      ...
      // Decompress
      guard
        let decompressedPayload = CompressionUtil.decompress(
          compressedPayload, originalSize: originalSize)
      else {
        return nil
      }
...
// Binary encoding for BleMessage
extension BleMessage {
  func toBinaryPayload() -> Data? {
    ...
    if let recipientNickname = recipientNickname,
      let recipData = recipientNickname.data(using: .utf8)
    {
  ...
  static func fromBinaryPayload(_ data: Data) -> BleMessage? {
    ...
    let id =
      String(data: dataCopy[offset..<offset + idLength], encoding: .utf8) ?? UUID().uuidString
    offset += idLength
    ...
    let sender =
      String(data: dataCopy[offset..<offset + senderLength], encoding: .utf8) ?? "unknown"
    offset += senderLength
    ...
    let contentLength = Int(
      contentLengthData.reduce(0) { result, byte in
        (result << 8) | UInt16(byte)
      })
    offset += 2
